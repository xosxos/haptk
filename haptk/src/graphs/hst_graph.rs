...
#[derive(Debug)]
pub struct HstGraph<'a> {
    pub g: &'a Graph<bhst::Node, u8>,
...
impl<'a> HstGraph<'a> {
    pub fn new(
        ...
        for idx in g.node_indices() {
            ...
            let block_len = bhst::calculate_block_len(vcf, node) as f32;
            if block_len > largest {
                largest = block_len;
            }
        }
        ...
        if min_width > s.width {
            tracing::warn!("Width of the HST graph has been incremented to {min_width}.");
            s.width = min_width;
        }

        let min_height = find_height(g, &s, min_size);
        if min_height > s.height {
            tracing::warn!("Height of the HST graph has been incremented to {min_height}.");
            s.height = min_height;
        }
    ...
    fn recursive_draw(
        ...
        for (i, (node_idx, node, startn, stopn)) in split_positions.into_iter().enumerate() {
            ...
            let additional_y = self.s.font_size * 1.2;
...
fn find_width(g: &Graph<bhst::Node, u8>, s: &GraphArgs, min_size: usize) -> f32 {
    let leaf_node_n = g
        .node_indices()
        .filter(|idx| {
...
fn find_height(g: &Graph<bhst::Node, u8>, s: &GraphArgs, min_size: usize) -> f32 {
