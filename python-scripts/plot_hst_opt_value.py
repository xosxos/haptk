...
ax = fig.subplots(2, 1)
df["logfisher"] = -np.log10(df["p.value"])
...
ax[1].scatter(df["POS"], df.logfisher, color="red", s=4, alpha=.90)
...
for i, axis in enumerate(ax):
    ax[i].set_xlim(xmin=0, xmax=df["POS"].max())
    ...
    if i == 1:
        ymax = max(10, df.logfisher.max())

        ax[i].axhline(y=8, ls='--', c='#696969')
        try:
            ax[i].set_ylim(ymin=0, ymax=ymax)
        except:
